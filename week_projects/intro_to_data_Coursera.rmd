...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(mean_ad = median(arr_delay), iqr_ad = IQR(arr_delay)) %>%
  arrange(desc(iqr_ad))
```
...
nycflights %>%
  group_by(origin) %>%
  summarise(ot_dep_rate = 100*sum(dep_type == "on time") / n()) %>%
  arrange(desc(ot_dep_rate))
```
...
nycflights %>%
  group_by(dep_type) %>%
  summarise(ot_arr_perc = 100*sum(arr_type == "on time") / n())
```
